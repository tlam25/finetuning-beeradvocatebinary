"""
Train T5-large + LoRA cho BeerAdvocate binary classification (1 aspect × 1 seed).
...
LƯU Ý T5-LARGE:
- T5-large fp16 rất không ổn định (hay ra NaN loss) do cách T5 scale attention.
  Khuyến nghị: `--bf16` cho A100/H100/RTX30+; fp32 (default) cho T4/V100.
- Target modules cho LoRA: ["q", "v"] (T5 dùng tên module khác BART).

Ví dụ chạy:
    torchrun --nproc_per_node=2 src/train_t5.py \
        --aspect appearance --seed 999 \
        --output_root /workspace/outputs \
        --wandb_project BeerAdvocate-T5Large-LoRA \
        --bf16
"""
...
def parse_args():
    ...
    # Model
    p.add_argument("--checkpoint", default="t5-large")
    ...
    p.add_argument("--wandb_project",
                   default="BeerAdvocate-T5Large-LoRA-4aspects",)
...
def main():
    ...
    lora_config = LoraConfig(
        ...
        target_modules=["q", "v"],  # T5 dùng tên "q","v" thay vì "q_proj","v_proj" như BART.
